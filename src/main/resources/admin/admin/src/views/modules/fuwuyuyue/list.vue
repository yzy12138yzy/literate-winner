<template>
	<div class="main-content" :style='{"color":"#666","padding":"30px","fontSize":"16px"}'>
			...
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","flexWrap":"wrap","justifyContent":"space-between","display":"flex"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"margin":"10px 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('fuwuyuyue','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
						<span class="icon iconfont icon-chujia3" :style='{"margin":"0 0px","fontSize":"inherit","color":"inherit","height":"34px"}'></span>
						批量支付
					</el-button>
				</el-row>
			</el-form>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('fuwuyuyue','取消预约')" @click="quxiaoyuyueCrossAddOrUpdateHandler(scope.row,'cross','否','否','[1]','已取消')" type="success">
								<span class="icon iconfont icon-qita3" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","height":"40px"}'></span>
								取消预约
							</el-button>
							<el-button class="btn8" v-if="isAuth('fuwuyuyue','服务分配')" @click="fuwufenpeiCrossAddOrUpdateHandler(scope.row,'cross','否','是','[1]','已分配')" type="success">
								<span class="icon iconfont icon-qita3" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","height":"40px"}'></span>
								服务分配
							</el-button>
		...
		<fuwufenpei-cross-add-or-update v-if="fuwufenpeiCrossAddOrUpdateFlag" :parent="this" ref="fuwufenpeiCrossaddOrUpdate"></fuwufenpei-cross-add-or-update>
...
<script>
	...
	import fuwufenpeiCrossAddOrUpdate from "../fuwufenpei/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				fuwufenpeiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			fuwufenpeiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptPay=='是'&&row.ispay!='已支付'){
					this.$message({
						message: "请支付完成再操作",
						type: "success",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.fuwufenpeiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','fuwuyuyue');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.fuwufenpeiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.fuwufenpeiCrossaddOrUpdate.init(row.id,type);
				});
			},
			payHandler(row){
