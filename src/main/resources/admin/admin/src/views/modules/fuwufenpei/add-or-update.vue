	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="预约说明" prop="yuyueshuoming" >
					<el-input v-model="ruleForm.yuyueshuoming" placeholder="预约说明" clearable  :readonly="ro.yuyueshuoming"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="预约说明" prop="yuyueshuoming" >
					<el-input v-model="ruleForm.yuyueshuoming" placeholder="预约说明" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="input" v-else-if="ruleForm.yuangonggonghao" label="员工工号" prop="yuangonggonghao" >
		...
		methods: {
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					this.ruleForm.fuwuzhuangtai = '未服务'; 				}
				// 获取用户信息
				this.$http({
				...
				this.$http({
					url: `option/baojieayi/yuangonggonghao`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
			...
			yuangonggonghaoChange () {
				this.$http({
					url: `follow/baojieayi/yuangonggonghao?columnValue=`+ this.ruleForm.yuangonggonghao,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `fuwufenpei/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.fuwufenpeiCrossAddOrUpdateFlag = false;
											this.parent.search();
