	...
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-button class="list-add-btn" v-if="btnAuth('jiazhengfuwu','新增')" type="primary" @click="add('/index/jiazhengfuwuAdd')">
					添加
				</el-button>
			</el-form>
			<div class="select2">
			...
			<div class="list">
				...
				<div class="list1 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
						...
						<div class="name">服务价格:{{item.fuwujiage}}</div>
						<div class="time_item">
...
<script>
	export default {
		...
		async created() {
			...
			await this.$http.get('option/fuwuleixing/fuwuleixing').then(res => {
				if (res.data.code == 0) {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`jiazhengfuwu/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/jiazhengfuwuDetail', query: params});
			},
