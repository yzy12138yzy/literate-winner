	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/fuwuyuyue?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">预约说明</div>
						<div class="text "  >{{detail.yuyueshuoming}}</div>
					</div>
					<div class="item">
					...
					<div class="item" v-if="btnAuth('fuwuyuyue','支付')">
						<div class="lable">是否支付</div>
						<div class="text">{{detail.ispay=='已支付'?'已支付':'未支付'}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('fuwuyuyue','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('fuwuyuyue','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('fuwuyuyue','取消预约')" 
							@click="onAcross('quxiaoyuyue','否','否','[1]','已取消')" type="warning">
							取消预约
						</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('fuwuyuyue','服务分配')" 
							@click="onAcross('fuwufenpei','否','是','[1]','已分配')" type="warning">
							服务分配
						</el-button>
						<el-button class="payBtn" v-if="(detail.ispay=='未支付'||!detail.ispay)&&btnAuth('fuwuyuyue','支付')" type="warning" size="small" @click="payClick">支付</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				localStorage.setItem('crossTable',`fuwuyuyue`);
			...
			payClick(){
				localStorage.setItem('paytable','fuwuyuyue')
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/fuwuyuyue', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此服务预约？') .then(_ => {
					this.$http.post('fuwuyuyue/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
