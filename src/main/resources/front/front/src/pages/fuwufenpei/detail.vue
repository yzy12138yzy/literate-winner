	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/fuwufenpei?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">员工工号</div>
						<div class="text "  >{{detail.yuangonggonghao}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">服务状态</div>
						<div class="text "  >{{detail.fuwuzhuangtai}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('fuwufenpei','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('fuwufenpei','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('fuwufenpei','服务记录')" 
							@click="onAcross('fuwujilu','','','fuwuzhuangtai','已服务','已服务,未服务'.split(',')[0])" type="warning">
							服务记录
						</el-button>
					</div>
				</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`fuwufenpei`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/fuwufenpei', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/fuwufenpeiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此服务分配？') .then(_ => {
					this.$http.post('fuwufenpei/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
