		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				<el-form-item class="list-item">
					...
					<el-input v-model="formSearch.yuangonggonghao" placeholder="员工工号" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
				...
				<el-button class="list-add-btn" v-if="btnAuth('baojieayi','新增')" type="primary" @click="add('/index/baojieayiAdd')">
					添加
				</el-button>
			</el-form>
			<div class="select2">
...
<script>
	export default {
		...
		async created() {
			...
			await this.$http.get('option/jiazhenggongsi/dianpumingcheng').then(res => {
				if (res.data.code == 0) {
					this.dianpumingchengOptions = res.data.data;
					this.selectOptionsList.push({name:'店铺名称',list:this.dianpumingchengOptions,tableName: 'dianpumingcheng',check: -1})
				}
			});
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				let searchWhere = {};
				if (this.formSearch.yuangonggonghao != '') searchWhere.yuangonggonghao = '%' + this.formSearch.yuangonggonghao + '%';
				...
				this.$http.get(`baojieayi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/baojieayiDetail', query: params});
			},
