<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="评价状态" prop="pingjiazhuangtai">
				<el-select v-model="ruleForm.pingjiazhuangtai" placeholder="请选择评价状态" :disabled="true  ||ro.pingjiazhuangtai" >
					<el-option
			...
			<el-form-item class="add-item" label="服务情况" prop="fuwuqingkuang">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="服务情况"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`fuwujilu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`fuwujilu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
