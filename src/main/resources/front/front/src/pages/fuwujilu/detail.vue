	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/fuwujilu?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">服务情况</div>
						<div class="text "  >{{detail.fuwuqingkuang}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">评价状态</div>
						<div class="text "  >{{detail.pingjiazhuangtai}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('fuwujilu','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('fuwujilu','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('fuwujilu','服务评价')" 
							@click="onAcross('fuwupingjia','','','pingjiazhuangtai','已评价','已评价,未评价'.split(',')[0])" type="warning">
							服务评价
						</el-button>
					</div>
				</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`fuwujilu`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/fuwujilu', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/fuwujiluAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此服务记录？') .then(_ => {
					this.$http.post('fuwujilu/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
