	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/fuwupingjia?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
			...
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.fuwumingcheng}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">服务效率</div>
						<div class="text "  >{{detail.fuwuxiaolv}}</div>
					</div>
					<div class="item">
						<div class="lable">服务态度</div>
						<div class="text "  >{{detail.fuwutaidu}}</div>
					</div>
					<div class="item">
						<div class="lable">服务质量</div>
						<div class="text "  >{{detail.fuwuzhiliang}}</div>
					</div>
					<div class="item">
						<div class="lable">评价建议</div>
						<div class="text "  >{{detail.pingjiajianyi}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('fuwupingjia','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('fuwupingjia','删除')" @click="delClick">删除</el-button>
						<el-button class="auditBtn" v-if="btnAuth('fuwupingjia','审核')" @click="onSH" type="warning">回复</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/fuwupingjia', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此服务评价？') .then(_ => {
					this.$http.post('fuwupingjia/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
